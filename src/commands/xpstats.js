...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('xpstats')
    .setDescription('View a detailed XP card.'),
  async execute(interaction) {
    const user = interaction.user;
    const guildId = interaction.guild.id;
    const totalXP = xpDB.getXP(user.id, guildId);
    const monthly = xpDB.getMonthlyXP(user.id, guildId);
    ...
    ctx.font='22px Sans';
    ctx.fillText(
      `Total XP: ${totalXP} (Monthly: ${monthly})`,
      aX+aSize+40,aY+80
    );
    ctx.fillText(
      `Level ${level}: ${xpIntoLevel}/${xpNeeded} XP`,
      aX+aSize+40,aY+110
    );
